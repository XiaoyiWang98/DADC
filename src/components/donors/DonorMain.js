...
import {
  DONATED_ITEMS,
  NGO_PROCESSED_SCHEDULES,
} from "../../tests/dummy_history";
...
class DonorMain extends Component {
  ...
  getHistory = () => {
    ...
    return this.props.isLoggedIn ? (
      <DonorHistorySection
        full_history={this.state.donorItems}
        isLoad={this.state.isLoadingHistory}
        info={this.state}
        updateInfo={this.updateInfo}
        updateList={this.collectMyItem}
      />
    ) : (
      <Redirect to="/donors/home" />
    );
            ...
            <Switch>
              ...
              <Route
                render={() => (
                  <DonorHome
                    info={this.state}
                    collectMyItem={this.collectMyItem}
                  />
                )}
              />
            </Switch>
