            ...
            renderItem={(donations) => (
              <List.Item>
                <List.Item.Meta
                  ...
                  description={
                    <div>
                      <div>Describtion: {donations.description}</div>
                ...
                {donations.status == 2 ? (
                  ...
                  <div className="delete_button_box">
                    <div className="donation-pending-text">
                      {PENDING_MSG}{" "}
                      <DeleteTwoTone
                        twoToneColor="#02a95c"
                        size="large"
                        onClick={() => {
                          this.showModal(donations);
                        }}
                      />{" "}
                    </div>
                    <Modal
                    ...
                    >
                      <p>
                        Regret about your donation? Someone really needs it!
                      </p>
                    </Modal>
                  </div>
                ) : (
    ...
    fetch(`${API_ROOT}/donor/delete_item?item_id=${donations.itemID}`, {
      ...
      .then((data) => {
        ...
        if (data.result === "SUCCESS") {
          ...
          const newList = this.props.filtered_history.filter(
            (item) => item.itemID != donations.itemID
          );
