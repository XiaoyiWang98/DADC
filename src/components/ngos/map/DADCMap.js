...
class NormalMap extends Component {
  ...
  render() {
    ...
    this.props.items.forEach((item) => {
      if (
        JSON.parse(this.props.hoverIds)[item.itemId] &&
        JSON.parse(this.props.selectedIds)[item.itemId]
      ) {
        itemMarkers.push(
          <ItemMarker
            key={item.itemId}
            item={item}
            colorlvl="select-attention"
          />
        );
      } else if (JSON.parse(this.props.hoverIds)[item.itemId]) {
        itemMarkers.push(
          <ItemMarker key={item.itemId} item={item} colorlvl="attention" />
        );
      } else if (JSON.parse(this.props.selectedIds)[item.itemId]) {
        itemMarkers.push(
          <ItemMarker key={item.itemId} item={item} colorlvl="selected" />
        );
      } else {
        itemMarkers.push(
          <ItemMarker key={item.itemId} item={item} colorlvl="regular" />
        );
      }
    });
    const routeMarkers = [];
    ...
    return (
      <GoogleMap defaultZoom={12} defaultCenter={this.props.mapCenter}>
