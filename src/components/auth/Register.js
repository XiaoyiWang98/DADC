...
class RegistrationForm extends Component {
    ...
    validateToNextPassword = (rule, value, callback) => {
        ...
        if (value && value.length < 8){
            ...
            callback('Password require numbers');
        }else if(value && !(/(?=.*[a-z])/.test(value))){
            callback('Password require lowercase letters');
        }else if(value && !(/(?=.*[A-Z])/.test(value))){
            callback('Password require uppercase letters');
        } else if(value && !(/(?=.*[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?])/.test(value))){
            callback('Password require special character');
        }  else {
