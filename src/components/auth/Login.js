      ...
      if (!err) {
        this.authenticate(values.username, values.password)
          .then((data) => {
            ...
            if (
              (data.idToken.payload["custom:custom:NGO"] == 0 &&
                this.props.userNGO == false) ||
              (data.idToken.payload["custom:custom:NGO"] == 1 &&
                this.props.userNGO == true)
            ) {
      ...
      <Form onSubmit={this.handleSubmit} className="login-form">
        ...
        <Form.Item>
          ...
          {this.props.userNGO ? (
            <span>
              Switch to{" "}
              <Link className="link" onClick={this.props.switchToDonor}>
                Resident Account
              </Link>
            </span>
          ) : (
            <span>
              Switch to{" "}
              <Link className="link" onClick={this.props.switchToNGO}>
                Ngo Account
              </Link>
            </span>
          )}
